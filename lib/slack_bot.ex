defmodule Worker.SlackBot do
  ...
  defp send_acronyms([%Acronym{name: name, means: "", description: description} | acronyms], user, state) do
    send_acronym("#{name}: #{description}", user, state)
    send_acronyms(acronyms, user, state)
  end

  defp send_acronyms([%Acronym{name: name, means: means, description: ""} | acronyms], user, state) do
    send_acronym("#{name} stands for #{means}.", user, state)
    send_acronyms(acronyms, user, state)
  end
  defp send_acronyms([%Acronym{name: name, means: means, description: description} | acronyms], user, state) do
    send_acronym("#{name} stands for #{means}. Description: #{description}", user, state)
    send_acronyms(acronyms, user, state)
  end

  defp send_acronym(text, user, state) do
    %{"channel" => %{"id" => channel_id}} = @slack_web_im_api.open(user, %{token: bot_token(state)})
